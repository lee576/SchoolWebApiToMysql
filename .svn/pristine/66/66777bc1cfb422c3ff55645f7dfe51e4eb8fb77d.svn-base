...
#pragma warning restore CS0168 // 声明了变量“ex”，但从未使用过
        ...
        public static UpdateResult Update(MongodbHost host, T t, string id)
        {
            try
            {
                ...
                //要修改的字段
                var list = (from item in t.GetType().GetProperties()
                    where item.Name.ToLower() != "id"
                    select Builders<T>.Update.Set(item.Name, item.GetValue(t))).ToList();
        ...
        public static long Count(MongodbHost host, FilterDefinition<T> filter)
        {
            ...
            {
                var client = MongodbClient<T>.MongodbInfoClient(host);
#pragma warning disable CS0618 // “IMongoCollection<T>.Count(FilterDefinition<T>, CountOptions, CancellationToken)”已过时:“Use CountDocuments or EstimatedDocumentCount instead.”
                return client.Count(filter);
#pragma warning restore CS0618 // “IMongoCollection<T>.Count(FilterDefinition<T>, CountOptions, CancellationToken)”已过时:“Use CountDocuments or EstimatedDocumentCount instead.”
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public static async Task<long> CountAsync(MongodbHost host, FilterDefinition<T> filter)
        {
            ...
            {
                var client = MongodbClient<T>.MongodbInfoClient(host);
#pragma warning disable CS0618 // “IMongoCollection<T>.CountAsync(FilterDefinition<T>, CountOptions, CancellationToken)”已过时:“Use CountDocumentsAsync or EstimatedDocumentCountAsync instead.”
                return await client.CountAsync(filter);
#pragma warning restore CS0618 // “IMongoCollection<T>.CountAsync(FilterDefinition<T>, CountOptions, CancellationToken)”已过时:“Use CountDocumentsAsync or EstimatedDocumentCountAsync instead.”
            }
            catch (Exception ex)
            {
                throw ex;
