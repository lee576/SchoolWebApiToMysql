    ...
    public partial class tb_ContractInfoService : GenericService<tb_ContractInfo>, Itb_ContractInfoService
    {
        ...
        public SchoolCardFlowingViewModel FindSchoolCardFlowingInfo(string ali_user_Id, string stime)
        {
            using (var db = DbFactory.GetSqlSugarClient())
            {
                ...
                //string ali_user_Id = FindById(contractId).ali_user_Id;
                string sqlContratInfo = $"SELECT COUNT(DISTINCT school_code) FROM tb_ContractInfo " +
                    $"WHERE ali_user_Id='" + ali_user_Id + "' AND CONVERT(varchar(100),contractTime, 23) like '" + sTime + '%' + "'";
                var ContratSchoolCount = db.Ado.SqlQuery<int>(sqlContratInfo);
                info.SignCount = ContratSchoolCount[0];
                //IEnumerable<tb_ContractInfo> contractInfo = FindListByOrder(a => a.ali_user_Id == ali_user_Id);
                var sqlSchoolCode = $"SELECT DISTINCT (school_code)AS school_code FROM tb_ContractInfo " +
                    $"WHERE ali_user_Id='" + ali_user_Id + "' AND CONVERT(varchar(100),contractTime, 23) like '" + sTime + '%' + "'";
                ...
                foreach (var item in cInfoList)
                {
                    ...
                    string sqlCollarcardInfo = $"SELECT COUNT(*) FROM tb_school_user " +
                             $"WHERE card_state=1  AND school_id = '" + school_Code + "' AND CONVERT(varchar(100),create_time, 23) like '" + sTime + '%' + "'";
                    var CollarcardInfoCount = db.Ado.SqlQuery<int>(sqlCollarcardInfo);//领卡人数

                    string sqlUnCollarcardInfo = $"SELECT COUNT(*) FROM tb_school_user " +
                            $"WHERE card_state=0  AND school_id = '" + school_Code + "' AND CONVERT(varchar(100),create_time, 23) like '" + sTime + '%' + "'";
                    var UnCollarcardInfoCount = db.Ado.SqlQuery<int>(sqlUnCollarcardInfo);
                    info.unRegisteredCcount += UnCollarcardInfoCount[0];//未领卡人数

                    string sql = $"SELECT COUNT(DISTINCT x.merchantCode) AS ID ,  ISNULL(SUM(x.receivableAmount),0) AS receivableAmount" +
                        $" FROM tb_xiyun_notify x INNER JOIN dbo.tb_school_info si ON x.merchantCode=si.xiyunMCode " +
                        $"WHERE  si.School_Code ='" + school_Code + "' AND CONVERT(varchar(100),x.tradeFinishedTime, 23) like '" + sTime + '%' + "'";
                    var list = db.Ado.GetDataTable(sql);
                    ...
                    string sqlpayment = $"SELECT COUNT(DISTINCT tpi.schoolcode) AS id,ISNULL(SUM(t.receipt_amount),0) AS receiptAmount FROM tb_payment_record t  " +
                        ...
                        $"WHERE CONVERT(varchar(100),t.pay_time, 23) like '" + YYYYMM + '%' + "' AND tpi.schoolcode = '" + school_Code + "'";
